...
print "                           A MIGRATION TOOL for MSL3+DVR -> AMD+NS\n";
...
=head2 EXAMPLE
	...
	-url="https://yourAkamaiMSL3DVRhostname-vh.akamaihd.net/i/path/filename_,bitrate1,bitrate2,...,.mp4.csmil/master.m3u8" \
...
sub rewriteMasterM3U8 {
	...
	foreach $line (@string) {
		$m3u8="";
			...
			if ($line=~/^(https?:\/\/.*\.akamaihd\.net\/i\/)(.*)\/([\w\+\-]+)@([0-9]+)\/([\w\+\-]+[\-p]?\.m3u8)(\??.*)$/i) { $m3u8 = "$5";}
...
if ($playlist=~/^(https?:\/\/.*\.akamaihd\.net\/i\/)(.*)\/([\w\+\-]+)@([0-9]+)\/([\w\+\-]+[\-p]?)\.m3u8(\??.*)$/i)
{
	$url_baseurl = $1;
	$url_hosturl = $1;
	$url_path = $2;
...
foreach $line (@master_m3u8) {
	...
	if ($line=~/^(https?:\/\/.*\.akamaihd\.net\/i\/)(.*)\/([\w\+\-]+)@([0-9]+)\/([\w\+\-]+[\-p]?)\.m3u8(\??.*)$/i)
	{
		$url_hosturl = $1;
		$url_path = $2;
		$url_track = $5;
...
foreach $subplaylist (@subplaylists) {
	...
	if ($subplaylist=~/^(https?:\/\/.*\.akamaihd\.net\/i\/)(.*)\/([\w\+\-]+)@([0-9]+)\/([\w\+\-]+[\-p]?)\.m3u8(\??.*)$/i)
	{
		$url_hosturl = $1;
		$url_path = $2;
		$url_track = $5;
...
foreach $subplaylist (@subplaylists) {
	...
	if ($subplaylist=~/^(https?:\/\/.*\.akamaihd\.net\/i\/)(.*)\/([\w\+\-]+)@([0-9]+)\/([\w\+\-]+[\-p]?)\.m3u8(\??.*)$/i)
	{
		...
		$url_event = $4;
		my $url_newPath = $url_filename.'_'.$url_track;
		$url_newFilename = $url_filename.'_'.$url_track;
		if ($debug)	{
			print "Found      : $line\n";
			print "  url      : $url_hosturl\n";
			print "  path     : $url_path\n";
	...
	if ($oktostart eq 1) {
		foreach $line (@child_m3u8) {
			if ($line=~/\.ts/ || $line=~/\.aac/) {
				...
				print INFOCOLOR, "Downloading $url_newFilename/$ts ", RESET;
				#my @cmd_seg = "";
